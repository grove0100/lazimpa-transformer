...
from .transformer import TransformerEncoder, TransformerEncoderImpatient, TransformerDecoder
...
class TransformerReceiverImpatient(nn.Module):
    # A CHECKER ########################################################################################################
    def __init__(self, agent, vocab_size, max_len, embed_dim, num_heads, hidden_size, num_layers, n_features, positional_emb=True,
                causal=True,causal_inv = False):
        super(TransformerReceiverImpatient, self).__init__()
        self.agent = agent
        self.hidden_to_output = nn.Linear(embed_dim, n_features)
        self.encoder = TransformerEncoderImpatient(vocab_size=vocab_size,
                                          max_len=max_len,
                                          embed_dim=embed_dim,
                                          num_heads=num_heads,
                                          num_layers=num_layers,
                                          hidden_size=hidden_size,
                                          positional_embedding=positional_emb,
                                          causal=causal,causal_inv=causal_inv)
    ...
    def forward(self, message, input=None, lengths=None):
        ...
        for step in range(transformed.size(0)):
            h_t = transformed[step,:,:]
            step_logits = F.log_softmax(self.hidden_to_output(h_t), dim=1)
            distr = Categorical(logits=step_logits)
            entropy.append(distr.entropy())
...
class TransformerSenderReinforce(nn.Module):
    ...
    def forward(self, x):
        ...
        return sequence, logits, entropy
diff --git a/test MLM/reinforce_wrappers.py b/test MLM/reinforce_wrappers.py
# Copyright (c) Facebook, Inc. and its affiliates.
